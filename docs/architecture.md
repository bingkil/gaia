...
flowchart LR
    ...
    EMSC & USGS & GEOFON & GDACS & FIRMS --> Adapters
    Adapters --> Raw
    Adapters --> DB
    Adapters --> Pipeline
    Pipeline --> DB
...
| `runtime.py` | Wires the above into one `Runtime` per process: owns the database, the pipeline, and the adapter tasks; the API reads through it and the ingestion loops write through it. Handles adapter start/stop, manual refresh with per-provider rate limiting, and raw-payload replay. |
| `api/` | `app.py` is the FastAPI app factory (lifespan starts/stops the `Runtime`, mounts the built frontend, serves `/healthz`); `routes.py` is the `/v1` REST surface (events, map GeoJSON, impact calculator, volcano catalogue, watch areas, notifications, provider health, manual VAA ingest, manual provider refresh, settings); `realtime.py` is the `/v1/realtime` WebSocket gateway, which reconnects clients from a sequence number and tells them to resynchronise via REST if the gap is too large for the in-memory backlog. |
...
- `map/MapView.tsx` renders the MapLibre GL map: `layers.ts` and `icons.ts` draw
  ...
  real-time basemap, and `useMapClock.ts` drives the time-scrubbing playhead.
- `components/` holds the UI chrome: `TopBar`, `ClockBar`, `Legend`,
  `EventList`/`EventDetail`, `NotificationFeed`, `ProviderHealthPanel`,
  `SettingsModal`, `VaaIngestPanel`, `WatchAreaPanel`, `ProvenanceBadge` (renders
  only the labels the backend's provenance enum permits), `ImageryAgeBadge`.
